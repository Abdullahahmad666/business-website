...
| `npm run cards` | Re-render the link-preview cards in `public/og/`. Needs Chrome or Edge; commit the PNGs. |
...
docs/                       README assets
tools/
  og-card.html              Template for the link-preview cards
  build-cards.mjs           Renders one card per route (`npm run cards`)
  prerender.mjs             Vite plugin: writes a static shell per route
public/
  asuppal/                  Product, category and warehouse imagery
  og/                       Generated 1200x630 link-preview cards, committed
src/
  ...
  data/
    ...
    catalogue.ts            Classes, lines and products, keyed by URL segment
    seo.ts                  Per-route title, description and card copy
  lib/
...
### Per-route metadata and link previews

The app is client-rendered, so without help every URL serves the same
`index.html` — and Facebook, X, LinkedIn, Slack, Discord and WhatsApp all read the
head of that document without ever running the bundle. Sharing
`/products/copper-scrap` produced the homepage's title, description and image.
Tags written from React cannot fix this; only bytes already in the HTML count.

[`src/data/seo.ts`](src/data/seo.ts) holds one record per route, derived from
`company.ts` and `catalogue.ts` rather than retyped, so renaming a product
renames it in that product's search result and shared card. At build time
[`tools/prerender.mjs`](tools/prerender.mjs) stamps a copy of `index.html` per
route with that record's tags swapped in between the `seo:start` / `seo:end`
markers. The body is untouched — React still boots and routes on the URL — so
this is head-only prerendering, not SSR. Page content remains client-rendered.

Cards are 1200x630 PNGs in `public/og/`, rendered from
[`tools/og-card.html`](tools/og-card.html) by headless Chrome. They are
generated by `npm run cards` and **committed**, deliberately outside
`npm run build`: the deploy builder has no browser, so generating them at
deploy time would break every deploy. Re-run `npm run cards` and commit the
PNGs after changing card copy.

When a custom domain goes live, change `SITE.origin` in `src/data/seo.ts` —
every canonical, `og:url` and `og:image` across all routes follows from it.
...
The build also writes a static shell per route (see
[Per-route metadata](#per-route-metadata-and-link-previews)), as both
`dist/about.html` and `dist/about/index.html`. The rewrite must stay a *fallback*: Vercel
gives the filesystem precedence over rewrites, so a request for `/about` finds its own
shell and only unrecognised paths fall through to `index.html`. A host that rewrites
before checking the filesystem would serve the homepage's card on every route again.
